...
// ErrAlreadyExists is returnes when a record with the same ID already exists in the store
...
type LaptopStore interface {
	...
	// Search searches for laptop whit filter, returns one by one via the found function
	Search(ctx context.Context, filter *pb.Filter, found func(laptop *pb.Laptop) error) error
}

// InMemoryLaptopStore is an store Laptop in memory
...
func isQualified(filter *pb.Filter, laptop *pb.Laptop) bool {
	...
	if toBit(laptop.GetMemery()) < toBit(filter.GetMinRam()) {
		return false
	}
	return true
}
